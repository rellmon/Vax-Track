...
<title>@yield('title', 'VaxTrack') — VaxTrack</title>
{{-- ⚡ MUST be first — sets theme + bg before ANY paint, killing all flicker --}}
<script>
(function(){
...
{{-- ── Mobile responsive styles (scoped to max-width: 768px, desktop untouched) ── --}}
{{-- ╔══════════════════════════════════════════════════════╗ --}}
...
{{-- Injected by mobile-responsive component; hidden on desktop via CSS --}}
...
<nav class="pill-sidebar no-print" id="mainSidebar">
  ...
  {{-- Nav links injected by child views --}}
  ...
  <div class="pill-dm-row" onclick="toggleDarkMode()" title="Toggle dark mode">
    <span class="icon" id="dmIcon">
      {{-- Moon icon — swapped to sun by JS in dark mode --}}
  ...
  <div class="pill-avatar-row">
    <div class="pill-avatar">@yield('user-initials', 'DR')</div>
    <div>
      <div class="pill-user-name">@yield('user-name', 'User')</div>
      <div class="pill-user-role">@yield('user-role', '')</div>
    </div>
  </div>
  ...
  <div class="topbar-wrap no-print" id="topbarWrap">
    <div class="topbar">
      <div class="topbar-left">
        ...
        <span class="topbar-role-pill">@yield('sidebar-role', 'Dashboard')</span>
      </div>
      <div class="flex items-center gap-3">
        <span class="topbar-badge">{{ now()->format('D, M j, Y') }}</span>
        ...
        <div class="topbar-user">
          <div class="topbar-avatar">@yield('user-initials', 'DR')</div>
          <div>
            <div class="topbar-user-name">@yield('user-name', 'User')</div>
            <div class="topbar-user-role">@yield('user-role', '')</div>
          </div>
        </div>
      </div>
    </div>
