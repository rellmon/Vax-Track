...
<div class="card mb-4">
  <div class="card-header">
    ...
    <span class="badge badge-teal">{{ $smsLogs->count() }} messages</span>
    @if($smsLogs->count() > 0)
      <form action="{{ route('doctor.schedules.sms-logs.clear') }}" method="POST" style="display:inline;" onsubmit="return confirm('Clear all SMS logs? This cannot be undone.');">
        @csrf
  ...
  <div class="card-body">
    <div class="sms-log">
      @forelse($smsLogs as $sms)
        <div class="sms-entry">
          <span class="sms-time">[{{ $sms->created_at->format('M j, g:i A') }}]</span>
          <span class="sms-ok">✓ SMS {{ strtoupper($sms->type) }}</span>
          <span class="sms-type">[{{ $sms->type }}]</span>
          → {{ $sms->phone }} — "{{ Str::limit($sms->message, 80) }}"
        </div>
      @empty
...
<div class="page-toolbar">
  <div class="page-toolbar-left">
    <form method="GET" action="{{ route('doctor.schedules') }}" style="display:flex;align-items:center;gap:8px;">
      <div class="search-bar" style="width:280px;">
        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
        <input type="text" name="search" placeholder="Search by child name..." value="{{ request('search') }}">
      </div>
      ...
      <tbody>
        @forelse($schedules as $s)
          <tr>
            ...
            <td>{{ $s->vaccine?->name }} <span class="badge badge-blue">{{ $s->vaccine?->type }}</span></td>
            <td>{{ \Carbon\Carbon::parse($s->appointment_date)->format('M j, Y') }}</td>
            <td>{{ \Carbon\Carbon::parse($s->appointment_time)->format('g:i A') }}</td>
            <td>
              <form action="{{ route('doctor.schedules.status', $s) }}" method="POST" style="display:inline;">
                @csrf @method('PUT')
            ...
            <td><span class="badge {{ $s->sms_sent?'badge-green':'badge-gray' }}">{{ $s->sms_sent ? 'Sent' : 'Pending' }}</span></td>
            <td>
              <div class="flex gap-2">
                <form action="{{ route('doctor.schedules.sms', $s) }}" method="POST">
                  @csrf
                ...
                <form action="{{ route('doctor.schedules.destroy', $s) }}" method="POST" onsubmit="return confirm('Delete this schedule?')">
                  @csrf @method('DELETE')
                  <button type="submit" class="btn btn-sm btn-danger" style="display:inline-flex;align-items:center;gap:5px;">
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/></svg>
                  </button>
      ...
      <div class="modal-body">
        <div class="form-group">
          ...
          <select name="child_id" class="form-control" required>
            ...
            @foreach($children as $c)<option value="{{ $c->id }}">{{ $c->full_name }}</option>@endforeach
          </select>
        </div>
        <div class="form-group">
          <label>Vaccine</label>
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input type="date" name="appointment_date" class="form-control" value="{{ request('date', today()->addDay()->format('Y-m-d')) }}" required>
          </div>
          <div class="form-group">
            <label>Time</label>
