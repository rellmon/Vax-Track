...
<div class="page-toolbar">
  <div class="page-toolbar-left">
    <form method="GET" action="{{ route('doctor.vaccines') }}" style="display:flex;align-items:center;gap:8px;">
      <div class="search-bar" style="width:280px;">
        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
        <input type="text" name="search" placeholder="Search vaccines..." value="{{ request('search') }}">
      </div>
        ...
        @forelse($vaccines as $v)
          <tr>
            <td>
              ...
              @if($v->description)
                <div class="text-sm text-muted">{{ Str::limit($v->description, 55) }}</div>
              @endif
            </td>
            ...
            <td><span class="badge {{ $v->active ? 'badge-green' : 'badge-gray' }}">{{ $v->active ? 'Active' : 'Inactive' }}</span></td>
            <td>
              <div class="flex gap-2">
                <a href="{{ route('doctor.vaccines.edit', $v) }}" class="btn btn-sm btn-secondary" style="display:inline-flex;align-items:center;gap:5px;">
                ...
                <form action="{{ route('doctor.vaccines.destroy', $v) }}" method="POST" onsubmit="return confirm('Delete this vaccine?')">
                  @csrf @method('DELETE')
...
<div class="modal-overlay" id="modal-add" onclick="if(event.target===this)this.classList.remove('open')">
  <div class="modal modal-md">
    ...
    <form action="{{ route('doctor.vaccines.store') }}" method="POST">
      @csrf
