...
@section('page-title', 'Register Child — Existing Parent')
@section('content')
<div style="max-width:680px;">
  ...
  <form action="{{ route('doctor.children.store-existing') }}" method="POST">
    @csrf
    <div class="card mb-4">
      ...
      <div class="card-body">
        <div class="form-group">
          ...
          <select name="parent_id" class="form-control" required>
            ...
            @foreach($parents as $p)<option value="{{ $p->id }}">{{ $p->full_name }} ({{ $p->phone }})</option>@endforeach
          </select>
        </div>
      </div>
    </div>
